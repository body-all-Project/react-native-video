...
- (void)itemPanGesture:(UIPanGestureRecognizer *)gesture
{
...
//    bool isXTrashArea = trashCenterPosition - 40 < nextXPosition && trashCenterPosition + 40 > nextXPosition;
    ...
    switch (gesture.state) {
        case UIGestureRecognizerStateBegan: {
            ...
            CGPoint locationInSuperView = [gesture locationInView:gestureView.superview];

            gestureView.layer.anchorPoint = CGPointMake(location.x /gestureView.bounds.size.width, location.y / gestureView.bounds.size.height);
...
- (void)itemPinchGesture:(UIPinchGestureRecognizer *)gesture
{
    if (_isActiveText) return;
...
- (void)itemRotationGesture:(UIRotationGestureRecognizer *)gesture
{
    UIView *gestureView = gesture.view;
    if (_isActiveText) return;
        ...
        case UIGestureRecognizerStateEnded: {
            if (_currentText) {
                ...
                if ([_currentText.text length] == 0 ) {
                    [_currentText removeFromSuperview];
                    return;
                }
                _isActiveText = false;
...
- (void)itemLongPressGesture:(UILongPressGestureRecognizer *)gesture
{
    if (_isActiveText) return;
...
//            addText.adjustsFontSizeToFitWidth = YES;
    ...
    // pan gesture
    UIPanGestureRecognizer *textPanGestureRecognizer = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(itemPanGesture:)];
    textPanGestureRecognizer.delegate = self;
    [addText addGestureRecognizer:textPanGestureRecognizer];


    // pinch
    UIPinchGestureRecognizer *textPinchGestureRecognizer = [[UIPinchGestureRecognizer alloc] initWithTarget:self action:@selector(itemPinchGesture:)];
    textPinchGestureRecognizer.delegate = self;
    [addText addGestureRecognizer:textPinchGestureRecognizer];

    // rotation
    UIRotationGestureRecognizer *textRotationGestureRecognizer = [[UIRotationGestureRecognizer alloc] initWithTarget:self action:@selector(itemRotationGesture:)];
    textRotationGestureRecognizer.delegate = self;
    [addText addGestureRecognizer:textRotationGestureRecognizer];

    // longpress stickerLongPressGesture
    UILongPressGestureRecognizer *textLongPressGestureRecognizer = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(itemLongPressGesture:)];
    textLongPressGestureRecognizer.delegate = self;
    textLongPressGestureRecognizer.minimumPressDuration = 0.02;
    [addText addGestureRecognizer:textLongPressGestureRecognizer];
...
- (void)setSticker:(NSDictionary *)sticker {
  _sticker = sticker;
    ...
    imgView.frame = CGRectMake((self.frame.size.width - (imgView.frame.size.width / 2)) / 2, (self.frame.size.height - (imgView.frame.size.height / 2))/ 2, 130, 130);
    ...
    // pan gesture
    UIPanGestureRecognizer *stickerPanGestureRecognizer = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(itemPanGesture:)];
...
- (void)setupPipController {
  if (!_pipController && _playerLayer && [AVPictureInPictureController isPictureInPictureSupported]) {
    // Create new controller passing reference to the
...
- (void) setVideoComposition:(UIView *)mergeView
{
...
//        UIImageView *imageView = self.subviews[0];
        [mergeView addSubview:subView];
    }

    [self setVideoComposition:mergeView];

    AVAsset *videoAsset = _playerItem.asset;

  if (videoAsset != nil) {

    AVAssetExportSession *exportSession = [AVAssetExportSession
                                           exportSessionWithAsset:videoAsset presetName:AVAssetExportPresetHighestQuality];
